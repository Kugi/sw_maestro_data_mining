    ...
    def __ifError(word):
        ...
        if tempRes == 0:
            ...
            while True:
                ...
                tempRes = PMICrawler.__search('네이버')
                if tempRes == 0:
                    continue;
                else:
                    try:
...
res2 = res[2000:]#res.ix[res['score']>50]
print 'size:'
print res.shape
res3 = res2['name']
print res3
...
key = load('/home/kugi/sw_maestro_data_mining/utils/data/app_info2.df')
keywords = [""]
keywords2 = [""]
for c in key[0]:
...
for a in searchWordArray:
    ...
    for key in keyword:
        sys.stdout.write(" [app %s  key %s] " % (a, key))
