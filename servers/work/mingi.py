...
class AppResource():
        ...
        for user in users:
            ...
            if gender:
            	if gender == 1:
                    ...
                    print "ERROR IN 'get_gender()'"
            else:
                print 'NAN'
        return {"male" : male, "female":female};

    def get_count_age(self, app):
	user_age = pickle.load(file('data/profiled_user_gender_ages_.df'))
	one = 0.0
	two = 0.0
	three = 0.0
	four = 0.0	
	...
	users = uapp[uapp['entity_id']==app].user_id
       	...
       	for user in users:
            age = user_age[user_age['user']==user]['age']
            if age:
            	if age == 1:
                    one += 1
                elif age == 2:
